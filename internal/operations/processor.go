...
func (p *Processor) Process(operationID string) error {
	...
	root, err := filepath.Abs(filepath.Join(p.outputRoot, service.Descriptor.Metadata.Name))
	if err != nil {
		return err
	}
	links := map[string]string{
		"repository": fileURL(root),
...
func (p *Processor) render(service domain.ServiceRecord, _ domain.Operation) error {
	...
	files := map[string]string{
		...
		"CODEOWNERS":               fmt.Sprintf("* @%s\n", strings.TrimPrefix(descriptor.Metadata.Owner, "@")),
	}
...
func generatedReadme(descriptor domain.ServiceDescriptor) string {
	...
	return fmt.Sprintf("# %s\n\nGenerated by the Internal Developer Platform from `%s`.\n\nOwner: `%s`\n\n## Run locally\n\n```sh\n%s\n```\n\nThe service exposes `/healthz`, `/readyz`, and `/metrics` on port `%d`.\n\n## Build a container\n\n```sh\ndocker build -t %s:local .\ndocker run --rm -p %d:%d %s:local\n```\n\n## Deploy\n\nUpdate the image reference in `deploy/kubernetes.yaml` to an image published by your CI, then apply the manifest. Review environment and resource requirements before using it outside development.\n", descriptor.Metadata.Name, descriptor.Spec.Template, descriptor.Metadata.Owner, runCommand, descriptor.Spec.Runtime.Port, descriptor.Metadata.Name, descriptor.Spec.Runtime.Port, descriptor.Spec.Runtime.Port, descriptor.Metadata.Name)
}
  ...
  ingress:
    - from:
      ...
      ports:
        - protocol: TCP
          port: %d
`, name, name, descriptor.Spec.Runtime.Replicas, name, name, name, descriptor.Spec.Runtime.Port, name, name, descriptor.Spec.Runtime.Port, name, name, descriptor.Spec.Runtime.Port)
}
